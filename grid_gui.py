...
# The "Grid" class is responsible for rendering the grid and user interactions.
class GridCanvas(tkinter.Canvas):
    ...
    # Draws the grid based on the current grid map
    ...
    # Sets the start point when the left mouse button is clicked
    ...
    # Sets the goal point when the right mouse button is clicked
    ...
    # Draws a point (either start or goal) on the canvas
    ...
    # Converts canvas click coordinates to grid coordinates
    ...
    # Clears the path from the canvas
    def clear_path(self):
        self.delete("path")


# The GUI class for controlling the grid navigation
class GridNavigationGUI(tkinter.Frame):
    ...
    # Searches for the path and displays it on the canvas
...
# Function to load the grid map from a file
    ...
    with open(grid_map_file) as infile:
        for row_index, row_data in enumerate(infile, start=1):
            ...
            for col_index, char in enumerate(row_data.strip(), start=1):
                if char == ".":
                    grid_map[-1].append(False)  # Empty space
                elif char == "X":
                    grid_map[-1].append(True)  # Obstacle
                else:
